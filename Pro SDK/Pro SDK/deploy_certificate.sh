...
do
   ...
   IP=$(echo ${words[5]})
   IPs=${IP}\,${IPs}
   #Transporting certificates and Docekr cert creation
   echo "- Creating certificate folder in worker node $IP"
   ssh -n -i ~/.ssh/$cluster -o "StrictHostKeyChecking no" "$WN_USER@$IP" "sudo mkdir -p -m 777 /etc/docker/certs.d/$WN_FOLDER_CERT/"  < /dev/null
